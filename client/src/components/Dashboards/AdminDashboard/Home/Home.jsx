...
function Home() {
  ...
  const getRequests = async () => {
    const hostel = JSON.parse(localStorage.getItem("hostel"));
    console.log(hostel);
    const res = await fetch(`${import.meta.env.VITE_BASE_URL}/api/messoff/list`, {
      method: "POST",
      headers: {
  ...
  const [suggestions, setSuggestions] = useState([
    {
      id: 1,
      title: "Steve Rogers [ Room: 201 ]",
      desc: "from 28-5-2024 to 29-5-2024",
    },
    {
      id: 1,
      title: "Thor Odinson [ Room: 201 ]",
      desc: "from 28-5-2024 to 29-5-2024",
    },
  ]);
  const messIcon = (
  ...
  return (
    <div className="w-full h-screen flex flex-col gap-3 items-center justify-center max-h-screen overflow-x-hidden overflow-y-auto pt-20">
      ...
      <div className="flex w-full gap-5 sm:px-20 pt-5 flex-wrap items-center justify-center">
        <ShortCard title="Total Students" number={457} />
        <ShortCard title="Total Complaints" number={24} />
        <ShortCard title="Total Suggestions" number={6} />
      </div>
