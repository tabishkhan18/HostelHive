...
function Attendance() {
  const getALL = async () => {
    setProgress(30);
    const marked = await fetch(`${import.meta.env.VITE_BASE_URL}/api/attendance/getHostelAttendance`, {
      method: "POST",
      headers: {
  ...
  const markAttendance = async (id, isPresent) => {
    const data = await fetch(`${import.meta.env.VITE_BASE_URL}/api/attendance/mark`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
          ...
          datasets: [
            {
              ...
              backgroundColor: ["#16A34A", "#DC2626", "#525252"],
              barThickness: 20,
              borderRadius: 0,
