...
function Invoices() {
  const genInvoices = async () => {
    ...
    try {
      const res = await fetch(`${import.meta.env.VITE_BASE_URL}/api/invoice/generate`, {
        method: "POST",
        headers: {
  ...
  const approveInvoice = async (id) => {
    setProgress(30);
    try {
      const res = await fetch(`${import.meta.env.VITE_BASE_URL}/api/invoice/update`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json"
        },
  ...
  const getInvoices = async () => {
    ...
    let hostel = JSON.parse(localStorage.getItem("hostel"));
    try {
      const res = await fetch(`${import.meta.env.VITE_BASE_URL}/api/invoice/getbyid`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json"
        },
