...
const List = () => {
  ...
  useEffect(() => {
    ...
    fetch(`${import.meta.env.VITE_BASE_URL}/api/invoice/student`, {
      method: "POST",
      headers: {
...
function Home() {
  ...
  const getAttendance = async () => {
    let student = JSON.parse(localStorage.getItem("student"));
    const res = await fetch(`${import.meta.env.VITE_BASE_URL}/api/attendance/get`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
  ...
  return (
    <div className="w-full h-screen flex items-center justify-center flex-col gap-5 max-h-screen overflow-y-auto">
