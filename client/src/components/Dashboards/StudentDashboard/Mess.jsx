...
function Mess() {
  let requestMessOff = async (event) => {
    ...
    let response = await fetch(`${import.meta.env.VITE_BASE_URL}/api/Messoff/request`, {
      method: "POST",
      headers: {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    if (student) {
      fetch(`${import.meta.env.VITE_BASE_URL}/api/Messoff/count`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  return (
    <div className="w-full h-screen gap-10 flex flex-col items-center justify-center max-h-screen overflow-y-auto pt-20">
        ...
        <div className="h-[30vh] gap-2 flex items-center justify-center flex-wrap">
          ...
          <ul className="text-white">
            <li className="flex gap-2">
              {" "}
              <span className="w-10 h-5 bg-red-600 block"></span> Mess Off
            </li>
            <li className="flex gap-2">
              {" "}
            ...
            <li className="flex gap-2">
              {" "}
              <span className="w-10 h-5 bg-green-600 block"></span> Mess
              Attended
            </li>
          </ul>
        </div>
