...
function Complaints() {
  ...
  const registerComplaint = async (e) => {
    ...
    const res = await fetch(`${import.meta.env.VITE_BASE_URL}/api/complaint/register`, {
      method: "POST",
      headers: {
  ...
  useEffect(()=> {
    ...
    const fetchComplaints = async () => {
      const res = await fetch(`${import.meta.env.VITE_BASE_URL}/api/complaint/student`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  return (
    <div className="w-full h-screen flex flex-col gap-10 items-center justify-center md:p-0 px-10 max-h-screen overflow-y-auto pt-8 lg:p-0">
      ...
      <div className="flex gap-5 flex-wrap items-center justify-center">
        ...
        >
          <div>
            ...
            <select
              className="border sm:text-sm rounded-lg block w-full p-2.5 bg-neutral-700 border-neutral-600 placeholder-neutral-400 text-white focus:ring-blue-500 focus:border-blue-500 outline-none"
              onChange={chngType}
            >
              {types.map((type) => (
